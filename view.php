...
$codeId = $codeData['id'];
$viewQuery = "SELECT * FROM views WHERE code_id = :code_id";
$viewStmt = $db->prepare($viewQuery);
$viewStmt->bindParam(':code_id', $codeId);
$viewStmt->execute();
...
if ($viewData) {
    ...
    $updateViewQuery = "UPDATE views SET views = :views WHERE code_id = :code_id";
    $updateViewStmt = $db->prepare($updateViewQuery);
    $updateViewStmt->bindParam(':views', $views);
    $updateViewStmt->bindParam(':code_id', $codeId);
    $updateViewStmt->execute();
} else {
    // Insert new view count
    $views = 1;
    $insertViewQuery = "INSERT INTO views (code_id, views) VALUES (:code_id, :views)";
    $insertViewStmt = $db->prepare($insertViewQuery);
    $insertViewStmt->bindParam(':code_id', $codeId);
    $insertViewStmt->bindParam(':views', $views);
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div class="flex items-center space-x-2">
                    <span class="px-3 py-1 bg-dark text-gray-300 text-sm rounded-md flex items-center">
                        <i class="far fa-eye mr-1"></i> <?php echo $views; ?>
                    </span>
                </div>
            </div>
